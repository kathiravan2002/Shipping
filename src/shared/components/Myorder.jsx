...
import { apimyorderRegionfilter } from "../services/Apiorders/apiorders";
...
export function Myorder() {
  ...
  useEffect(() => {
    const fetchFilterOptions = async () => {
      try {
        const response = await apimyorderRegionfilter();
  ...
  const CONSIGNEE_STATUS = {
   ...
   const getDispatcheddate = (statusHistory) => {
    ...
    return dispatchedEntry ? new Date(dispatchedEntry.timestamp).toLocaleString().slice(0, 9) : "N/A";
  };
