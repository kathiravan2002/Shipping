...
const Addorder = () => {
  ...
  useEffect(() => {
    ...
    formData.consignees.forEach((consignee, index) => {
      if (consignee.consigneedistrict) {
        const originalDistrict = apiData
          .find((d) => toSentenceCase(d.district) === consignee.consigneedistrict)?.district;
        const filteredPincodes = apiData
          .filter((item) => item.district === originalDistrict)
          .map((item) => ({ pincode: item.pincode, officename: item.officename }));
    ...
    if (consigneeType === "Single" && singleConsignee.consigneedistrict) {
      const originalDistrict = apiData
        .find((d) => toSentenceCase(d.district) === singleConsignee.consigneedistrict)?.district;
      const filteredPincodes = apiData
        .filter((item) => item.district === originalDistrict)
        .map((item) => ({ pincode: item.pincode, officename: item.officename }));
      newPincodesMap["single"] = filteredPincodes;
    }
  }, [formData.consignees, singleConsignee.consigneedistrict, apiData, consigneeType]);
          ...
          if (Array.isArray(cosaveorder)) {
            ...
            if (cosaveorder.length === 1 && consigneeType === "Single") {
              setSingleConsignee({
                ...
                consigneedistrict: cosaveorder[0].consigneedistrict || "",
    ...
    try {
      ...
      if (consigneeType === "Single") {
        consigneesToSend = formData.consignees.map(consignee => ({
          ...
          consigneedistrict: singleConsignee.consigneedistrict,
        ...
        {consigneeType === "Single" && (
          <div className="mt-6">
            ...
            <div className="grid lg:grid-cols-5 gap-2">
              ...
              <select name="consigneedistrict" value={singleConsignee.consigneedistrict} onChange={handleSingleConsigneeChange} className="w-full p-4 border-2 bg-purple-50 rounded mb-2 focus:outline-none focus:ring-purple-400 focus:ring-2" required>
                <option value="">Select District</option>
                  ...
                  {formData.consignees.map((consignee, index) => (
                    <tr key={index}>
                      ...
                      <td className="border p-2 space-x-1">
                        <input type="text" value={consignee.typename} onChange={(e) => handleConsigneeChange(index, "typename", e.target.value)} placeholder="Product" className="w-1/2 border p-1 rounded bg-violet-50" required />
                        <select value={consignee.ptype} onChange={(e) => handleConsigneeChange(index, "ptype", e.target.value)} className="border p-1 rounded bg-violet-50 mt-1" required>
                      ...
                      <td className="border p-2"><input value={consignee.totalWeight ? `${consignee.totalWeight} Kg` : ""} className=" border p-1 rounded bg-violet-50"/></td>
                      ...
                      <td className="border p-2"><input  value={consignee.cprice ? `₹${consignee.cprice}` : ""} className="border p-1 rounded bg-violet-50"/></td>
            ...
            <table className="w-full border-collapse border border-gray-300 text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="border p-2 w-[100px]">Weight</th>
                ...
                {formData.consignees.map((consignee, index) => (
                  <tr key={index}>
                    ...
                    <td className="border p-2">
                      ...
                      <select value={consignee.consigneedistrict} onChange={(e) => handleConsigneeChange(index, "consigneedistrict", e.target.value)} className="w-full border p-1 rounded bg-violet-50 mt-1" required>
                    ...
                    <td className="border p-2"><input type="tel" value={consignee.weight} onChange={(e) => handleConsigneeChange(index, "weight", e.target.value)} placeholder="Weight" className="w-full border p-1 rounded bg-violet-50" required min="0" /></td>
                    <td className="border p-2"><input type="tel" value={consignee.packages} onChange={(e) => handleConsigneeChange(index, "packages", e.target.value)} placeholder="Qty" className="w-full border p-1 rounded bg-violet-50" required min="0" /></td>
                    <td className="border p-2"><input value={consignee.totalWeight ? `${consignee.totalWeight} Kg` : ""} className="w-full border p-1 rounded bg-violet-50"/></td>
                    <td className="border p-2"><input type="tel" value={consignee.cpriceperkg} onChange={(e) => handleConsigneeChange(index, "cpriceperkg", e.target.value)} placeholder="₹/kg" className="w-full border p-1 rounded bg-violet-50" min="0" /></td>
                    <td className="border p-2"><input value={consignee.cprice ? `₹${consignee.cprice}` : ""} className="w-full border p-1 rounded bg-violet-50"/></td>
                    <td className="border p-2">
                      <select value={consignee.cstatus} onChange={(e) => handleConsigneeChange(index, "cstatus", e.target.value)} className="w-full border p-1 rounded bg-violet-50" required>
                        <option value="">Select Status</option>
                        {getNextConsigneeStatuses(consignee.cstatus).map(status => (
