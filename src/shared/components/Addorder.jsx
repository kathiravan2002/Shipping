...
const Addorder = () => {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    const initialCurrentRegion = userRole === "admin" ? "" : userRegion;
    return {
      orderDetails: {
        ConsignerName: "",
        ...
        instruction: "",
        currentRegion: initialCurrentRegion,
        deliveryimage: null
      },
      consignees: [{
        cid: "",
    ...
    if (id) {
      const fetchOrderDetails = async () => {
        try {
          ...
          const userRole = localStorage.getItem("role") || "user";
          const userRegion = localStorage.getItem("Region") || "";
          const currentRegionForOrder = userRole === "admin" ? savedOrder.currentRegion || "" : userRegion;

          setFormData(prev => ({
            ...prev,
            orderDetails: {
              ...savedOrder,
              currentRegion: currentRegionForOrder
            },
          ...
          if (Array.isArray(cosaveorder)) {
            ...
            let newOrderStatus;
            if (cosaveorder.length === 1) {
              newOrderStatus = cosaveorder[0].cstatus;
            } else {
              const statuses = cosaveorder.map(co => co.cstatus);
              const allSame = statuses.every(status => status === statuses[0]);
              newOrderStatus = allSame ? statuses[0] : "Partial";
            }
  ...
  const handleConsigneeChange = (index, field, value) => {
    ...
    let newOrderStatus;
    if (updatedConsignees.length === 1) {
      newOrderStatus = updatedConsignees[0].cstatus;
    } else {
      const statuses = updatedConsignees.map(co => co.cstatus);
      const allSame = statuses.every(status => status === statuses[0]);
      newOrderStatus = allSame ? statuses[0] : "Partial";
    }
  ...
  const deleteConsignee = (index) => {
    ...
    let newOrderStatus;
    if (updatedConsignees.length === 0) {
      newOrderStatus = "Order Placed"; // Fallback if no consignees remain
    } else if (updatedConsignees.length === 1) {
      newOrderStatus = updatedConsignees[0].cstatus;
    } else {
      const statuses = updatedConsignees.map(co => co.cstatus);
      const allSame = statuses.every(status => status === statuses[0]);
        ...
        <div className="grid lg:grid-cols-5 gap-2 mt-4">
          ...
          {formData.orderDetails.Orderstatus === "Order Placed" && (
            localStorage.getItem("role") === "admin" ? (
              <select name="currentRegion" value={formData.orderDetails.currentRegion} onChange={handleOrderInputChange} className="p-4 border-2 rounded mb-2 bg-violet-50" required>
              ...
              <input
                type="text"
                value={formData.orderDetails.currentRegion}
                className="p-4 border-2 rounded mb-2 bg-violet-50"
                readOnly
              />
            )
          )}
